...
class RectangleAnnotation(Annotation):
    def __init__(self,
        ...
        :param track_id: unique id of object annotation belongs to
        :param track_idx: index of annotation in sequence of object annotations
        :param track_len: length of sequence of object annotations
        """
        self.shape = "rect"
        self.x = x
        self.y = y
        ...
        self.uid = uid
        self.seq_id = track_id
        self.seq_idx = track_idx
        self.seq_len = track_len
        self.frame_id = frame_id
        self.is_keypoint = track_is_keypoint
    ...
    @property
    def coords_int(self):
        coords = tuple(int(np.round(c)) for c in self.coords)
        return coords

    @property
    def bounds_int(self):
        bounds = tuple(int(np.round(c)) for c in self.bounds)
        return bounds

    def __str__(self):
        return f"{self.label} - x: {self.x}, y: {self.y}, w: {self.width}, h: {self.height}"
